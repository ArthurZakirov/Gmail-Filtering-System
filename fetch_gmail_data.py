...
from data.gmail_data_extractor import fetch_gmail_messages_as_df
...
def parse_arguments():
    ...
    parser.add_argument(
        "--after",
        type=str,
        help="Fetch emails after this date (format: YYYY/MM/DD). "
        "Overrides query if provided.",
    )

    parser.add_argument(
        "--before",
        type=str,
        help="Fetch emails before this date (format: YYYY/MM/DD). "
        "Overrides query if provided.",
    )
    ...
    parser.add_argument(
        ...
        "Overrides other date options if provided.",
    )
...
def fetch_gmail_data():
    ...
    if not os.path.exists(args.token):
        ...
        logging.error(
            "Please run gmail_auth_setup.py first to generate the token file."
        )
        sys.exit(1)
        return  # For test environments where sys.exit is mocked
    ...
    try:
        ...
        SCOPES = [
            urllib.parse.urljoin(SCOPES_ROOT, access_key)
            for access_key in scope_access_keys
        ]
