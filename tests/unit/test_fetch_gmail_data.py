...
class TestParseArguments:
    ...
    def test_date_arguments(self):
        ...
        with patch('sys.argv', ['fetch_gmail_data.py', '--after', '2025/01/01', '--before', '2025/01/31']):
            args = fetch_gmail_data.parse_arguments()
    ...
    def test_long_form_arguments(self):
        ...
        with patch('sys.argv', ['fetch_gmail_data.py', '--query', 'from:test@example.com', '--output', 'test.csv', '--token', 'custom.json', '--verbose']):
            args = fetch_gmail_data.parse_arguments()
            assert args.query == "from:test@example.com"
...
class TestFetchGmailDataFunction:
    ...
    def test_successful_data_fetch_flow(self, mock_parse_args, mock_exists, mock_creds, mock_build, mock_fetch_df):
    ...
    @patch('fetch_gmail_data.parse_arguments')
    @patch('fetch_gmail_data.sys.exit')
    def test_api_error_handling(self, mock_exit, mock_parse_args, mock_exists, mock_creds, mock_build, mock_fetch_df):
    ...
    @patch('fetch_gmail_data.parse_arguments')
    @patch('fetch_gmail_data.sys.exit')
    def test_file_not_found_error_handling(self, mock_exit, mock_parse_args, mock_exists, mock_creds, mock_build, mock_fetch_df):
